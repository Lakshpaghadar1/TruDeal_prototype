...
import com.trudeals.databinding.CustomToolbarBinding
import com.trudeals.databinding.CustomToolbarPopupMenuBinding
...
class CustomToolbar(context: Context, attrs: AttributeSet?) :
    ...
    private var location = typedArray.getString(R.styleable.CustomToolbar_location)
    ...
    private var maxItemAllowed = typedArray.getInteger(
        R.styleable.CustomToolbar_maxItemAllowed,
        DEFAULT_MAX_ITEM_ALLOWED
    )

    private var popupMenuElevation =
    ...
    // Handling of Toolbar location text
    private fun handleToolbarLocationText() = with(binding) {
        location?.let {
            showView(textViewToolbarLocation)
            textViewToolbarLocation.text = location
        } ?: run {
            hideView(textViewToolbarLocation)
        }
    }

    /**
     * Use to set the toolbar title
     * @param location Pass the title you want to display.
     *
     * */
    fun setToolbarLocation(location: String?): CustomToolbar {
    ...
    /**
     * Use to set the toolbar title
     * @param location Pass the StringRes which you want to display as title.
     * */
    fun setToolbarLocation(@StringRes location: Int): CustomToolbar {
        this.location = context.getString(location)

        return this
    }

    // Toolbar location text color
    private fun handleToolbarLocationTextColor() = with(binding) {
        textViewToolbarLocation.setTextColor(
            ResourcesCompat.getColor(
                resources,
                toolbarLocationTextColor,
                null
            )
        )
    }

    fun setToolbarLocationTextColor(@ColorRes toolbarLocationTextColor: Int): CustomToolbar {
    ...
    fun setAndDecorateToolbarLocationText(
        ...
        this.textDecoratorLocation = decorateToolbarLocationText(
            TextDecorator.decorate(
                binding.textViewToolbarLocation,
                text
            )
        )
    ...
    private fun showPopupMenu() {
        ...
        popupWindow?.showAsDropDown(
            ...
            -(resources.getDimensionPixelOffset(R.dimen._180sdp)),
    ...
    private fun setMenuItems() {
        ...
        if (menuItemList.size - menuItemsToShowOnPopupMenu.size > maxItemAllowed) {
            ...
            menuItemsToShowOnPopupMenu
                .addAll(menuItemsToShowOnToolbarIfRoom.takeLast(itemsToFind))
            ...
            menuItemsToShowOnToolbar
                .addAll(menuItemsToShowOnToolbarIfRoom)
        }
        ...
        if (menuItemsToShowOnPopupMenu.isNotEmpty()) {
            ...
            menuItemAdapter.addItem(
                MenuItem(
                    ...
                    icon = moreOptionIcon,
                    tag = DefaultMenuItemTag.MoreVert
                )
            )
        }
    }
    ...
    private fun setPaddingOnToolbarTitle() = with(binding) {
        post {
            val padding =
                recyclerViewOptionsMenu.measuredWidth + resources.dpToPx(R.dimen._10sdp)
    ...
    fun build() {
        ...
        handleToolbarLocationText()
        handleToolbarLocationTextColor()
        showHamburgerIcon()
        setUpOptionsMenu()
        setMenuItems()
    ...
    companion object {
        ...
        const val DEFAULT_HAMBURGER_ICON = R.drawable.ic_menu
